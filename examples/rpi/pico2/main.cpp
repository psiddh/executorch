  ...
  if (!method_result.ok()) {
    ...
    switch(method_result.error()) {
      ...
      case Error::NotSupported:
        ...
        printf("   → This usually means missing operators in selective build!\n");
        break;
      case Error::NotFound:
...
bool run_inference(Method& method) {
  printf("🔥 ExecuTorch MLP MNIST Demo 🔥\n");
  printf("Neural network on $4 microcontroller!\n\n");
  
  // ASCII art for digit '7' (28x28)
  ...
  for(int row = 0; row < 28; row++) {
    for(int col = 0; col < 28; col++) {
      char pixel = ascii_digit_7[row][col];
  ...
  if (result != Error::Ok) {
      ...
      blink_indicator(INDICATOR_PIN_1, 10, 100);
      return false;
  }
  ...
  for(int i = 0; i < 10; i++) {
    ...
    if(i == predicted_digit) printf(" ← PREDICTED");
    printf("\n");
  }
  
  printf("\n🎉 PREDICTED DIGIT: %d\n", predicted_digit);
  printf("🎉 PyTorch neural network works on Pico2!\n");
