...
Enable `ET_ARM_MODEL_PTE_DMA_ACCESSIBLE` to skip the SRAM copy.

### Pattern 2: Model SRAM copy overflows RAM

**Symptom:** RAM overflows by roughly the .pte file size.

**Cause:** Without `ET_ARM_MODEL_PTE_DMA_ACCESSIBLE`, main.cpp creates
`model_pte_runtime[sizeof(model_pte)]` — a writable copy of the entire model in SRAM.

**Fix:** If the model is in DMA-accessible memory (DDR, MRAM), enable
`CONFIG_ET_ARM_MODEL_PTE_DMA_ACCESSIBLE=y` in the board conf.
