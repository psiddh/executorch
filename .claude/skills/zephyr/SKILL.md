...
pip install west cmake pyelftools ninja jsonschema
...
Copy `executorch.yaml` into the submanifests directory:

```bash
cp modules/lib/executorch/zephyr/executorch.yaml zephyr/submanifests/
```

Or for local development, symlink your ET checkout:
...
ET requires three memory pools at runtime. These are sized via Kconfig:

| Pool | Kconfig | Default | Purpose |
|------|---------|---------|---------|
| Method allocator | `CONFIG_EXECUTORCH_METHOD_ALLOCATOR_POOL_SIZE` | 1.5 MiB | Planned buffers + input/output tensors |
| Temp allocator | `CONFIG_EXECUTORCH_TEMP_ALLOCATOR_POOL_SIZE` | 1.5 MiB | Delegate scratch (e.g., Ethos-U scratch buffer) |
| Fast scratch | `ET_ARM_BAREMETAL_FAST_SCRATCH_TEMP_ALLOCATOR_POOL_SIZE` | 1.5 KiB | Small Ethos-U fast memory |
...
When `ET_ARM_MODEL_PTE_DMA_ACCESSIBLE` is enabled, the runtime skips copying the model blob to a writable SRAM buffer. Use this when the model already resides in DMA-accessible memory (DDR, MRAM).
...
| `backends/arm/scripts/pte_to_header.py` | Converts .pte to C header with section attribute |
